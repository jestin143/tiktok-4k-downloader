...
let MEMORY_CFG = load();
function save(c){
  MEMORY_CFG = JSON.parse(JSON.stringify(c)); // FIX 2: DEEP COPY PARA HINDI MABURA ADS!
  try{
...
// FIX 3: DITO TALAGA SIRA - DAPAT MERGE HINDI DIRECT PALIT!
app.post('/api/admin/config',auth,(req,res)=>{
  const c=process.env.NODE_ENV==='production'?MEMORY_CFG:load();
  const b=req.body;
  if(b.ads){
    // HINDI c.ads=b.ads LANG - DAPAT ISA ISA!
    if(b.ads.page1) c.ads.page1.code = b.ads.page1.code || '';
    if(b.ads.page2) c.ads.page2.code = b.ads.page2.code || '';
    if(b.ads.modal){
      c.ads.modal.code = b.ads.modal.code || '';
      if(b.ads.modal.height_4k) c.ads.modal.height_4k = b.ads.modal.height_4k;
      if(b.ads.modal.height_720) c.ads.modal.height_720 = b.ads.modal.height_720;
    }
  }
  if(b.site) c.site=b.site;
  if(b.downloads) c.downloads=b.downloads;
  if(b.greetings) c.greetings=b.greetings;
  if(b.admin) c.admin=b.admin;
  save(c);
  console.log('ADS SAVED OK:', c.ads.page1.code.length, c.ads.page2.code.length);
...
app.post('/api/download', async (req, res) => {
  ...
  try {
    if (url.includes('vt.tiktok.com') || url.includes('vm.tiktok.com') || url.includes('tiktok.com/t/')) {
      try{
        const r = await axios.get(url, {maxRedirects:5,timeout:8000,headers:{'User-Agent':'Mozilla/5.0 (iPhone; CPU iPhone OS 16_0 like Mac OS X) AppleWebKit/605.1.15 Version/16.0 Mobile/15E148 Safari/604.1'},httpsAgent:agent});
    ...
    const response = await axios.get(apiUrl, {httpsAgent:agent,timeout:15000,headers:{'User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36','Accept':'application/json','Referer':'https://www.tikwm.com/'}});
    if (response.data && response.data.data && response.data.data.play) {
      const d = response.data.data;
      return res.json({title:d.title||'TikTok Video',author:d.author?(d.author.nickname||d.author.unique_id||'@creator'):'@creator',cover:d.cover||d.origin_cover||d.ai_dynamic_cover||'',play:d.play||'',hdplay:d.hdplay||d.play||''});
    }
